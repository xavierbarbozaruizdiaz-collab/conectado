...
// ✅ Hooks/singletons propios
import { useStorage, useUser, useFirestore, useCollection } from '@/firebase';

// ✅ Helpers Firestore desde el paquete correcto
import { collection, addDoc, doc, query, where, orderBy } from 'firebase/firestore';

// ✅ Hook de documento propio
import { useDoc } from '@/firebase/firestore/use-doc';

// ✅ Storage helpers desde el paquete correcto
...
export default function AddProductPage() {
  ...
  const productsQuery = useMemo(
    () =>
      user
        ? query(collection(firestore!, 'products'), where('sellerId', '==', user.uid))
        : null,
    [user, firestore],
  );
  const { data: sellerProducts, loading: productsLoading } =
  ...
  const tiersQuery = useMemo(
    () => (firestore ? collection(firestore!, 'subscriptionTiers') : null),
    [firestore],
  );
  const { data: tiers, loading: tiersLoading } =
    useCollection<SubscriptionTier>(tiersQuery);

  // ✅ doc() con segmentos separados ('config', 'platform')
  const settingsDocRef = useMemo(
    () => (firestore ? doc(firestore, 'config', 'platform') : null),
    [firestore],
  );
  const { data: platformSettings, loading: settingsLoading } =
    useDoc<PlatformSettings>(settingsDocRef);
    ...
    return (
      <div className="flex flex-col items-center justify-center h-full text-center p-8">
        ...
        <p className="text-muted-foreground mt-2 max-w-md">
          Has alcanzado el límite de {productLimit} productos para tu plan actual (
          {currentTier?.name}). Para seguir publicando, por favor, mejora tu suscripción.
        </p>
        ...
        <div className="lg:col-span-2 space-y-8">
              ...
              <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-5 gap-4">
                ...
                {images.length < 10 && (
                  <Label
                    ...
                    className="relative aspect-square rounded-md border-2 border-dashed flex flex-col items-center justify-center text-muted-foreground cursor-pointer hover:bg-muted/50 transition-colors"
                  >
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Opcional. Ofrece precios reducidos para compras en volumen. No aplica para subastas.
              </CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
              {wholesalePrices.map((tier, index) => (
                <div key={index} className="flex items-center gap-4 p-3 bg-muted rounded-md">
                  <Label className="text-sm" htmlFor={`ws-qty-${index}`}>
                    A partir de
                  </Label>
                  <Input
                    id={`ws-qty-${index}`}
                    ...
                    disabled={isAuction}
                  />
                  <Label className="text-sm" htmlFor={`ws-price-${index}`}>
                    el precio es
                  </Label>
                  <Input
                    id={`ws-price-${index}`}
          ...
          <Card>
            ...
            <CardContent className="space-y-6">
              <div className="flex items-center space-x-2">
                ...
                <Label htmlFor="sale-type-switch" className="text-lg font-medium">
                ...
                <div className="space-y-3 rounded-lg border bg-muted/50 p-4 text-sm">
                  ...
                  <div className="flex justify-between">
                    <span className="text-muted-foreground">
                      Comisión (
                      {isAuction
                        ? platformSettings.auctionSellerCommission
                        : platformSettings.directSaleCommission}
                      %)
                    </span>
                  ...
                  {isAuction && platformSettings.auctionBuyerCommission > 0 && (
                    <div className="text-xs text-muted-foreground pt-2">
                      ...
                      El precio final para el comprador (incluyendo su comisión del{' '}
                      {platformSettings.auctionBuyerCommission}%) será de aproximadamente{' '}
